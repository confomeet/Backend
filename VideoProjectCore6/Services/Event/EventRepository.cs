...
,
    ...
    public async Task<ListCount> GetAll_V0(int CurrentUserId, EventSearchObject obj = null, int pageIndex = 1, int pageSize = 25, string lang = "ar")
    {
        bool serverSearch = obj != null && obj.HasDateSearch();
        bool localSearch = obj != null && obj.HasStringSearch();
        var host = _IConfiguration["CurrentHostName"];
        List<int> idsList = new List<int>();
        if (localSearch && !string.IsNullOrWhiteSpace(obj.Participant))
        {
            idsList = await _DbContext.Events
            .Include(x => x.Participants).ThenInclude(u => u.User)
            .Where(x => !serverSearch || ((obj.StartDate == null || (x.StartDate.Date >= obj.StartDate.Value.Date && x.StartDate.Date <= obj.EndDate.Value.AddDays(1).Date)) || (obj.EndDate == null || (x.EndDate.Date >= obj.StartDate.Value.Date && x.EndDate <= obj.EndDate.Value.Date)) || (obj.StartDate.Value >= x.StartDate.Date && obj.EndDate.Value <= x.EndDate.Date))
            && (string.IsNullOrWhiteSpace(obj.Topic) || x.Topic.Contains(obj.Topic)) && (string.IsNullOrWhiteSpace(obj.SubTopic) || x.Topic.Contains(obj.SubTopic))
            && (x.Participants.Any(p => !string.IsNullOrWhiteSpace(p.User.FullName) && p.User.FullName.Contains(obj.Participant) || x.Participants.Any(p => !string.IsNullOrWhiteSpace(p.Email) && p.Email.ToLower().Contains(obj.Participant.ToLower())))))
            .OrderByDescending(e => e.StartDate)
            .Select(x => x.Id)
            .ToListAsync();
        }
        ...
        {
            idsList = await _DbContext.Events
                  .Where(x => !serverSearch || (obj.StartDate == null || (x.StartDate.Date >= obj.StartDate.Value.Date && x.StartDate.Date <= obj.EndDate.Value.AddDays(1).Date)) 
                  || (obj.EndDate == null 
                  || (x.EndDate.Date >= obj.StartDate.Value.Date && x.EndDate <= obj.EndDate.Value.Date)) 
                  || (obj.StartDate.Value >= x.StartDate.Date && obj.EndDate.Value <= x.EndDate.Date)
                   && (string.IsNullOrWhiteSpace(obj.Topic) || x.Topic.Contains(obj.Topic)) && (string.IsNullOrWhiteSpace(obj.SubTopic) || x.Topic.Contains(obj.SubTopic)))
                  .OrderByDescending(e => e.StartDate)
                  .Select(x => x.Id)
                  .ToListAsync();
        ...
        var filteredIds = PaginatedList<int>.Create(idsList.AsQueryable(), pageIndex > 0 ? pageIndex : 1, pageSize > 0 ? pageSize : 20, total);
        ...
        List<EventFullView> events = await _DbContext.Events.AsNoTracking()
            ...
            .Select(e => new EventFullView
            {
                ...
                MeetingLink = e.MeetingId != null && e.CreatedBy == CurrentUserId && e.ParentEvent == null ? ((e.Meeting.MeetingLog != null) ? e.Meeting.MeetingLog : Url.Combine(host, "join", e.Participants.Where(p => p.UserId == e.CreatedBy).Select(p => Url.Combine(p.Id.ToString(), p.Guid.ToString())).FirstOrDefault()) + "?redirect=0") : null,
                ParentEventId = e.ParentEvent,
                MeetingStatus = _IGeneralRepository.CheckStatus(e.StartDate, e.EndDate, e.Id, e.MeetingId, lang, allRooms),
                StatusText = e.RecStatus == null ? string.Empty : EventStatusValue.ContainsKey((EVENT_STATUS)e.RecStatus) ? EventStatusValue[(EVENT_STATUS)e.RecStatus][lang] : string.Empty,
                Participants = e.Participants.Select(p => new ParticipantView
        ...
        if (events != null && events.Any())
        {
            ...
            foreach (var e in events)
            {
                e.CreatedByName = userName[e.CreatedBy];
                foreach (var par in e.Participants)
                {
                    try
                    {
                        par.FullName = userName[par.UserId];
                    }
                    catch { }
                }
            }
    ...
    //public async Task<List<EventFullView>> GetPPEventByType(int userId, EventSearchObject obj, string lang = "ar") 
    //{
    //    bool hasDate= obj.HasDateSearch();
    //    if(obj.TypeOrder == null)
    //    {
    //        return null;
    //    }
    //    var et =await _DbContext.SysLookupTypes.Where(x => x.Value == "event_type").Select(x => x.Id).FirstOrDefaultAsync();
    //    var targetType = await _DbContext.SysLookupValues.Where(x => x.LookupTypeId == et && (obj.TypeOrder != 0 && x.Order == obj.TypeOrder)).Select(x => x.Id).FirstOrDefaultAsync();
        
    //    var host = _IConfiguration["CurrentHostName"];
    //    var events = await _DbContext.Events
    //                .Where(x =>             
    //                (!hasDate || (obj.StartDate == null || (x.StartDate >= obj.StartDate.Value.Date && x.StartDate < obj.EndDate.Value.AddDays(1).Date))
    //                || (obj.EndDate == null || (x.EndDate > obj.StartDate.Value.Date && x.EndDate <= obj.EndDate.Value.AddDays(1).Date))
    //                || (obj.StartDate.Value.Date >= x.StartDate && obj.EndDate.Value.Date <= x.EndDate)
    //                )
                    
    //                && ((obj.TypeOrder != 0 && targetType != 0) ? x.Type == targetType : true) 
    //                && (obj.ShowSubEvent || (x.ParentEvent == null))
    //                && (obj.OrderNo != null ? x.OrderNo == obj.OrderNo : true)).Select(e => new EventFullView
    ...
    //               TimeZone = e.TimeZone,
    //               Password = e.Meeting.Password,
    //               PasswordReq = e.Meeting != null ? e.Meeting.PasswordReq != false ? e.Meeting.PasswordReq : false : false,
    //               RecordingReq = e.Meeting.RecordingReq != null ? (bool)e.Meeting.RecordingReq : false,
    //               SingleAccess = e.Meeting.SingleAccess != null ? (bool)e.Meeting.SingleAccess : false,
    //               MeetingId = e.MeetingId,
    //               Status = e.RecStatus,
    //               Type = e.Type,
    //               AllDay = e.AllDay,
    //               MeetingLink = e.MeetingId != null ? Url.Combine(host, $"join?meetingId={e.MeetingId}") : null, //Here Public meeting Link
    //               ParentEventId = e.ParentEvent,
    //               //MeetingStatus = _IGeneralRepository.CheckStatus(e.StartDate, e.EndDate, e.Id, e.MeetingId, lang, allRooms),
    //               StatusText = e.RecStatus == null ? string.Empty : EventStatusValue.ContainsKey((EVENT_STATUS)e.RecStatus) ? EventStatusValue[(EVENT_STATUS)e.RecStatus][lang] : string.Empty,
    //           }).ToListAsync();
    //    return events;
    //}
    //private static int checkStatus(DateTime startDate, string meetingId)
    //{
